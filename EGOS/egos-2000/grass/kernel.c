...
static void intr_entry(uint id) {
    ...
    ulonglong elapsed_time = now - p->last_cpu_update_time;
    p->cpu_time += elapsed_time;
    mlfq_update_level(p, elapsed_time);   // Update position in the MLFQ

    p->last_cpu_update_time = now;        // Safety check
    
    // Reset MLFQ if MLFQ_RESET_TIME has elapsed
    mlfq_reset_level();
...
static void proc_yield() {
    ...
    int next_level = MLFQ_NLEVELS;   
    for (uint i = 1; i <= MAX_NPROCESS; i++) {
        struct process* p = &proc_set[(curr_proc_idx + i) % MAX_NPROCESS];
        // Complete any pending syscalls to make the process ready/runnable
        if (p->status == PROC_PENDING_SYSCALL) proc_try_syscall(p);


        // Finding the process with lowest MLFQ level
        ulonglong now = mtime_get();
        if (p->status == PROC_READY || p->status == PROC_RUNNABLE) {
            // Skip sleeping process
            if (p->sleep_until > now) continue;

            // Comparing to find the process with the lowest MLFQ level 
            if (p->mlfq_level < next_level) {
                next_level = p->mlfq_level;
    ...
    if (next_idx < MAX_NPROCESS) {
         ...
         * Modify mstatus.MPP to enter machine or user mode after mret. */
         ...
         // Upadting the new process to current process
        struct process* next_proc = &proc_set[next_idx];
        
        // Calulating first schedule time
        if (next_proc->first_scheduled_time == 0)
            next_proc->first_scheduled_time = mtime_get();

        // Calculating start of CPU run
        next_proc->last_cpu_update_time = mtime_get();
